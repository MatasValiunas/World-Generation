...
{
    public static float[,] Noise(int size, float maxHeight, int seed, float roughness, bool resize)
    {
        ...
        {
            float noise = Mathf.Pow(roughness, i);
            
            DiamondStep(heightmap, step, noise, size);

            SquareStep(heightmap, step, noise, size);
        }
